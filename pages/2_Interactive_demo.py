...
def main():
    ...
    with st.status('Downloading data', expanded=True) as status:
        ...
        status.update(label='Running trials')
        np.random.seed(42)
        ...
        status.update(label='Drawing plots')
        # fig = go.Figure()
        # df_scenario_outcomes_by_lsoa.set_index('LSOA', inplace=True)
        ...
        pan_offset_lon = 0.05
        pan_offset_lat = 0.025

        n=50
        # Works for one subplot:
        # steps = [{'args': [{'geo.lonaxis.range': [extent[0] + pan_offset_lon * w, extent[1] + pan_offset_lon * w]}],
        #                     'method': 'relayout'} for w in range(n-10)]

        lon_steps = [
            {
                'args': [{
                    'geo.lonaxis.range': [extent[0] + pan_offset_lon * w, extent[1] + pan_offset_lon * w],
                    'geo2.lonaxis.range': [extent[0] + pan_offset_lon * w, extent[1] + pan_offset_lon * w],
                    'geo3.lonaxis.range': [extent[0] + pan_offset_lon * w, extent[1] + pan_offset_lon * w],
                    'geo4.lonaxis.range': [extent[0] + pan_offset_lon * w, extent[1] + pan_offset_lon * w],
                    'geo5.lonaxis.range': [extent[0] + pan_offset_lon * w, extent[1] + pan_offset_lon * w],
                    'geo6.lonaxis.range': [extent[0] + pan_offset_lon * w, extent[1] + pan_offset_lon * w],
                    'geo7.lonaxis.range': [extent[0] + pan_offset_lon * w, extent[1] + pan_offset_lon * w],
                    'geo8.lonaxis.range': [extent[0] + pan_offset_lon * w, extent[1] + pan_offset_lon * w],
                    }],
                'method': 'relayout'
            }
            for w in np.arange(-20, 21)
            ]
        lat_steps = [
            {
                'args': [{
                    'geo.lataxis.range': [extent[2] + pan_offset_lat * w, extent[3] + pan_offset_lat * w],
                    'geo2.lataxis.range': [extent[2] + pan_offset_lat * w, extent[3] + pan_offset_lat * w],
                    'geo3.lataxis.range': [extent[2] + pan_offset_lat * w, extent[3] + pan_offset_lat * w],
                    'geo4.lataxis.range': [extent[2] + pan_offset_lat * w, extent[3] + pan_offset_lat * w],
                    'geo5.lataxis.range': [extent[2] + pan_offset_lat * w, extent[3] + pan_offset_lat * w],
                    'geo6.lataxis.range': [extent[2] + pan_offset_lat * w, extent[3] + pan_offset_lat * w],
                    'geo7.lataxis.range': [extent[2] + pan_offset_lat * w, extent[3] + pan_offset_lat * w],
                    'geo8.lataxis.range': [extent[2] + pan_offset_lat * w, extent[3] + pan_offset_lat * w],
                    }],
                'method': 'relayout'
            }
            for w in np.arange(-20, 21)
        ...
        fig.update_layout(sliders=[
            dict(  # Pan left/right
                ...
                minorticklen = 0,
                steps = lon_steps,
                currentvalue_prefix='Pan left/right: '
                ),
            dict(  # Pan up/down
                active = 20,
                minorticklen = 0,
                steps = lat_steps,
                y=-0.1,
                currentvalue_prefix='Pan up/down: '
                )
            ])

        # button1= dict(method = "relayout",
        #             args = [{
        #                 "geo.lonaxis.range": [fig['layout']['geo']['lonaxis']['range'][0] + pan_offset_lon, fig['layout']['geo']['lonaxis']['range'][1] + pan_offset_lon],
        #                 "geo2.lonaxis.range": [fig['layout']['geo2']['lonaxis']['range'][0] + pan_offset_lon, fig['layout']['geo2']['lonaxis']['range'][1] + pan_offset_lon],
        #                 "geo3.lonaxis.range": [fig['layout']['geo3']['lonaxis']['range'][0] + pan_offset_lon, fig['layout']['geo3']['lonaxis']['range'][1] + pan_offset_lon],
        #                 "geo4.lonaxis.range": [fig['layout']['geo4']['lonaxis']['range'][0] + pan_offset_lon, fig['layout']['geo4']['lonaxis']['range'][1] + pan_offset_lon],
        #                 "geo5.lonaxis.range": [fig['layout']['geo5']['lonaxis']['range'][0] + pan_offset_lon, fig['layout']['geo5']['lonaxis']['range'][1] + pan_offset_lon],
        #                 "geo6.lonaxis.range": [fig['layout']['geo6']['lonaxis']['range'][0] + pan_offset_lon, fig['layout']['geo6']['lonaxis']['range'][1] + pan_offset_lon],
        #                 "geo7.lonaxis.range": [fig['layout']['geo7']['lonaxis']['range'][0] + pan_offset_lon, fig['layout']['geo7']['lonaxis']['range'][1] + pan_offset_lon],
        #                 "geo8.lonaxis.range": [fig['layout']['geo8']['lonaxis']['range'][0] + pan_offset_lon, fig['layout']['geo8']['lonaxis']['range'][1] + pan_offset_lon],
        #                 }], 
        #             label = "Pan left"
        #         )
        ...
        #             args = [{
        #                 "geo.lataxis.range": [fig['layout']['geo']['lataxis']['range'][0] + pan_offset_lat, fig['layout']['geo']['lataxis']['range'][1] + pan_offset_lat],
        #                 "geo2.lataxis.range": [fig['layout']['geo2']['lataxis']['range'][0] + pan_offset_lat, fig['layout']['geo2']['lataxis']['range'][1] + pan_offset_lat],
        #                 "geo3.lataxis.range": [fig['layout']['geo3']['lataxis']['range'][0] + pan_offset_lat, fig['layout']['geo3']['lataxis']['range'][1] + pan_offset_lat],
        #                 "geo4.lataxis.range": [fig['layout']['geo4']['lataxis']['range'][0] + pan_offset_lat, fig['layout']['geo4']['lataxis']['range'][1] + pan_offset_lat],
        #                 "geo5.lataxis.range": [fig['layout']['geo5']['lataxis']['range'][0] + pan_offset_lat, fig['layout']['geo5']['lataxis']['range'][1] + pan_offset_lat],
        #                 "geo6.lataxis.range": [fig['layout']['geo6']['lataxis']['range'][0] + pan_offset_lat, fig['layout']['geo6']['lataxis']['range'][1] + pan_offset_lat],
        #                 "geo7.lataxis.range": [fig['layout']['geo7']['lataxis']['range'][0] + pan_offset_lat, fig['layout']['geo7']['lataxis']['range'][1] + pan_offset_lat],
        #                 "geo8.lataxis.range": [fig['layout']['geo8']['lataxis']['range'][0] + pan_offset_lat, fig['layout']['geo8']['lataxis']['range'][1] + pan_offset_lat],
        #                 }], 
        #             label="Pan up"
        #             )

        # button4= dict(method = "relayout",
        #             args = [{
        #                 "geo.lataxis.range": [fig['layout']['geo']['lataxis']['range'][0] - pan_offset_lat, fig['layout']['geo']['lataxis']['range'][1] - pan_offset_lat],
        #                 "geo2.lataxis.range": [fig['layout']['geo2']['lataxis']['range'][0] - pan_offset_lat, fig['layout']['geo2']['lataxis']['range'][1] - pan_offset_lat],
        #                 "geo3.lataxis.range": [fig['layout']['geo3']['lataxis']['range'][0] - pan_offset_lat, fig['layout']['geo3']['lataxis']['range'][1] - pan_offset_lat],
        ...
        #                                     buttons=[button1, button2, button3, button4, button1, button2, button3, button4],
        #                                 x=0.99, y=0.99, xanchor='right', yanchor='top')
        #                             ])

        # st.write(fig.layout)
        plotly_config={
            'modeBarButtonsToRemove': ['pan', 'lasso2d', 'select2d', 'reset'],
            'displaylogo': False,
            'displayModeBar': True
        }

        status.update(label='Plot created', state="complete", expanded=False)
    # st.plotly_chart(fig)
    st.plotly_chart(
        fig,
